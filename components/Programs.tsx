...
// 하나로 깔끔하게 합쳤습니다. (Target은 사용하지 않는다면 제외)
import {
    Lightbulb,
    Users,
    Zap,
...
const mainProgramGroup: ProgramGroup = {
    ...
    description:
        "공감과 상호이해를 바탕으로 조직 내 갈등을 해소하고 구성원 간의 연결을 돕는 '더 비저니스트'의 대표 교육 라인업입니다.",
    subPrograms: [
        {
            id: "Program 1-1",
            title: "나미야 잡화점의 기적",
            ...
            description:
                "역지사지의 마음으로 동료의 고민을 나누고, 편지를 통해 유쾌한 울림을 느끼는 소통 과정",
            ...
            modules: [
                ...
                },
                {
                    ...
                    content:
                        "캐릭터 프로파일링을 통한 공감지도 작성, 주인공의 입장에서 6가지 질문에 답하기",
                },
                {
                    title: "M3. 우리의 사연에 공감해요",
                    content:
                        "갤러리워크 방식의 전체 공유, 고민에 대한 답장 편지 작성 및 낭독",
                },
            ],
        },
        {
            ...
            description:
                "라디오와 음악을 활용한 아날로그 감성으로 서로의 이야기를 듣고 공감하는 소통 과정",
            icon: <Radio className="w-6 h-6 text-white" />,
            color: "bg-[#1A2B48]", // Unified Navy
            placeholderIcon: (
                <Music className="w-10 h-10 text-[#1A2B48] opacity-80" />
            ),
            ...
            modules: [
                {
                    ...
                    content:
                        "나를 표현하는 인생 곡 공유, 팀만의 유니크한 플레이리스트 선정 및 소개",
                },
                {
                    title: "M2. 디제이에게 사연을 보내요",
                    content:
                        "동료에게 전하고 싶은 마음을 담은 사연 작성 (익명/실명), 신청곡 선정",
                },
                {
                    title: "M3. 볼륨을 높여요",
                    content:
                        "일일 DJ의 사연 소개와 음악 감상, 공감과 지지의 메시지 나누기",
                },
            ],
        },
    ],
...
const otherPrograms: SimpleProgram[] = [
    {
        title: "창의적 혁신",
        description:
            "고정관념을 깨고 새로운 가치를 창출하는 창의적 사고와 혁신 문화를 조직에 심어줍니다.",
    ...
    },
    {
        title: "조직 시너지",
        description:
            "상호 존중과 효과적인 소통을 통해 최상의 팀워크를 발휘하는 고성능 조직을 만듭니다.",
    ...
    },
    {
        title: "미래 역량",
        description:
            "디지털 대전환 시대에 필수적인 핵심 역량을 함양하여 경쟁력 있는 인재로 거듭납니다.",
    ...
    return (
        <div className="space-y-3">
            {modules.map((module, index) => (
                <div
                    key={index}
                    className="bg-gray-50/80 rounded-lg p-4 border border-gray-100 hover:bg-gray-50 transition-colors"
                >
                    <h5 className="text-sm font-bold mb-1 text-[#1A2B48]">
                        {module.title}
                    </h5>
...
function ProgramGroupSection({ group }: { group: ProgramGroup }) {
    const [openSubProgramId, setOpenSubProgramId] = useState<string | null>(
        group.subPrograms[0].id,
    );
        ...
        <div className="mb-16 border border-gray-200 rounded-3xl overflow-hidden bg-white shadow-lg">
                ...
                <div className="relative z-10">
                    <div className="flex flex-wrap gap-2 mb-4">
                        {group.keywords.map((keyword, i) => (
                            <span
                                key={i}
                                className="px-3 py-1 bg-white/10 rounded-full text-xs font-medium text-brand-gold border border-white/10"
                            >
                                {keyword}
                            </span>
                        ))}
            ...
            <div className="divide-y divide-gray-100">
                {group.subPrograms.map((subProgram) => {
                    ...
                    const toggleOpen = () =>
                        setOpenSubProgramId(isOpen ? null : subProgram.id);

                    return (
                        <div
                            key={subProgram.id}
                            className={`transition-colors duration-300 ${isOpen ? "bg-white" : "hover:bg-gray-50"}`}
                        >
                            {/* Header (Clickable) */}
                            <button
                                onClick={toggleOpen}
                            ...
                            >
                                <div
                                    className={`hidden md:flex w-12 h-12 rounded-xl ${subProgram.color} items-center justify-center shrink-0 shadow-md group-hover:scale-105 transition-transform`}
                                >
                                    {subProgram.icon}
                                </div>
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-3">
                                        {subProgram.features.map((feature, i) => (
                                            <span
                                                key={i}
                                                className="text-xs text-[#1A2B48] font-medium opacity-80"
                                            >
                                                {feature}
                                            </span>
                                        ))}
                                ...
                                <div className="p-6 lg:p-8 pt-0 pl-0 md:pl-[5.5rem] grid lg:grid-cols-3 gap-6">
                                    ...
                                    <div className="lg:col-span-2">
                                        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                                            <div className="bg-gray-50 px-4 py-3 border-b border-gray-100">
                                                <span className="text-xs font-bold text-[#1A2B48] uppercase tracking-wider">
                                                    Curriculum Modules
                                                </span>
                                            </div>
                                            <div className="p-4">
                                    ...
                                    <div className="lg:col-span-1">
                                        <div className="h-full min-h-[200px] bg-gray-50 rounded-xl border border-gray-100 flex flex-col items-center justify-center relative overflow-hidden group">
                                            <div
                                                className={`absolute inset-0 opacity-5 ${subProgram.color}`}
                                            ></div>
            ...
            <div className="max-w-7xl mx-auto px-6">
                <div className="text-center mb-16">
                    ...
                    <p className="text-gray-600 max-w-2xl mx-auto leading-loose break-keep">
                        조직의 성과를 극대화하고 지속 가능한 성장을 이끄는 체계적인 교육
                        솔루션을 제안합니다.
                    </p>
                </div>
                ...
                <div ref={contentRef}>
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {otherPrograms.map((program, index) => (
                            ...
                            >
                                <div
                                    className={`w-14 h-14 rounded-xl ${program.color} flex items-center justify-center mb-6 shadow-lg group-hover:scale-110 transition-transform`}
                                >
                                    {program.icon}
                                </div>
